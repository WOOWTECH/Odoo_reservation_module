...
class AppointmentController(http.Controller):
    ...
    def _get_tz(appointment_type):
        ...
        try:
            return pytz.timezone(appointment_type.timezone or 'UTC')
        except pytz.UnknownTimeZoneError:
            return pytz.UTC

    @staticmethod
    def _local_to_utc(tz, naive_local):
        """Naive local wall time -> naive UTC (the value stored in DB).

        Uses tz.localize() rather than .replace(tzinfo=tz): pytz zones carry
        historical offsets (Asia/Taipei would resolve to LMT +08:06 with
        .replace()) and localize() also picks the right DST offset.
        """
        return tz.localize(naive_local).astimezone(pytz.utc).replace(tzinfo=None)

    @staticmethod
    def _utc_to_local(tz, naive_utc):
        """Naive UTC (as stored in DB) -> naive local wall time."""
    ...
    def _today_local(cls, appointment_type):
        ...
        user has no tz set, so it silently falls back to the UTC date.
        """
        return datetime.now(cls._get_tz(appointment_type)).date()

    @staticmethod
    def _hour_to_local_dt(day_start_local, hour_float):
        """Turn an availability Float hour into a naive local datetime.

        Handles hour_to == 24.0 (midnight of the next day), which the previous
        .replace(hour=24) implementation could not express.
        """
